...
function centro_servizi_render_settings_page(): void
{
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['centro_servizi_nonce'])) {
        ...
        // FONT
        update_option('centro_servizi_font_body', sanitize_text_field($_POST['font_body'] ?? ''));
        update_option('centro_servizi_font_headings', sanitize_text_field($_POST['font_headings'] ?? ''));
    ...
    $font_body = get_option('centro_servizi_font_body', 'arial');
    $font_headings = get_option('centro_servizi_font_headings', 'georgia');
